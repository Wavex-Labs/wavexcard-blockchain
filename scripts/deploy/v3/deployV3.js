...
async function main() {
    console.log("Starting V3 deployment process...");
    ...
    try {
        ...
        // Get the contract factory for WaveXNFTV3
        console.log("Creating WaveXNFTV3 contract factory...");
        const WaveXNFTV3 = await ethers.getContractFactory("WaveXNFTV3");

        console.log("Deploying WaveXNFTV3...");
        const wavexNFT = await WaveXNFTV3.deploy({
        ...
        console.log("WaveXNFTV3 deployed to:", deployedAddress);
        ...
        if (networkConfig.usdt) {
            ...
            console.log("Setting USDT as payment token for templates...");
            for (let i = 1; i <= 4; i++) {
                await wavexNFT.setTemplatePaymentToken(i, networkConfig.usdt, {
                    gasPrice: ethers.parseUnits("100", "gwei"),
                    gasLimit: 2000000
                });
            }
        ...
        const deploymentPath = path.join(
            deploymentsDir,
            `${network.name}_deploymentV3.json`
        );
        fs.writeFileSync(
            deploymentPath,
        ...
        if (network.name !== "hardhat" && network.name !== "localhost") {
            const confirmations = networkConfig.confirmations || 2;
